...
- The LSP binary is the **only** place tower-lsp/tokio/dashmap are used.
- Binaries set the mimalloc `#[global_allocator]` at their final link point
  (currently `llg` and `helloworld`; see `src/bin/llg/main.rs`).
...
- `llg` → `llg::core` (compile, db, model, tokens) + `llg::ffi`.
- `llg_sim` → `llg::core::compile` + `llg::sim` (codegen, rt).
