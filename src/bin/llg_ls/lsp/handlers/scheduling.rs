...
impl Backend {
    ...
    pub(super) fn compile_job(
        ...
        // the shadow-staging lock, so the previous job's admitted mirrors
        // from the previous job can be removed wholesale here. The
        // analysis itself parks the process CWD inside this directory (see
        // `features::analysis_scratch_dir`) so temporary files never
        ...
        let analysis = if files.is_empty() {
            ...
            // of allowing the frontend to read an unmeasured project file.
            let compilation_units: BTreeSet<_> = job.files.iter().collect();
            let source_paths: BTreeSet<_> = job.files.iter().chain(include_deps.iter()).collect();
            let sources = source_paths
                .into_iter()
                .map(|path| {
                    let text = input_snapshots
                        .text(path)
                        .expect("source path admitted before Slang request construction");
                    let name = path.to_string_lossy().into_owned();
                    if compilation_units.contains(path) {
                        llg::core::compile::OwnedSource::compilation_unit(name, text)
                    } else {
                        llg::core::compile::OwnedSource::include(name, text)
                    }
                })
                .collect();
            let opts = config::compile_opts_sources(&job.config, sources);
            if !Self::job_current(state, &job) {
                staging_span.as_mut().unwrap().outcome("stale");
                crate::llg_debug!(
