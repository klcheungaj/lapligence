...
pub(in crate::sim::emit_c) fn main(execution: &ExecutionModel) -> Result<String, String> {
    ...
    if model.waveform {
        ...
        for array in &model.arrays {
            for index in 0..array.total {
                ...
                let call = if array.real {
                    format!(
                        "llg_wave_register_real({}, &{}[{index}])",
                        c_string_literal(&name),
                        array.c_name
                    )
                } else {
                    format!(
                        "llg_wave_register_sv4({}, &{}[{index}], {})",
                        c_string_literal(&name),
                        array.c_name,
                        array.elem_width
                    )
    ...
    for (name, fallback_label) in model.spawn_list() {
        let process = execution
            .processes()
            .iter()
            .find(|item| model.processes[item.semantic_process].c_name == name);
    ...
    if model.waveform {
        out.push_str(
            "    llg_spawn_final(llg_wave_capture_final_time, \"llg.wave.capture_final_time\");\n",
        );
    }
    for name in &model.final_spawns {
        let label = model
            .processes
            .iter()
            .find(|p| &p.c_name == name)
            .map(|p| p.label())
            .unwrap_or(name);
        out.push_str(&format!(
            "    llg_spawn_final({name}, {});\n",
            c_string_literal(label)
        ));
    }
    out.push_str(
