...
fn context_for(model: &IrModel, helper: &str) -> Option<IrEventContext> {
    let helpers = model.processes.iter().flat_map(|process| process.pre_fns.iter())
        .chain(model.funcs.iter().flat_map(|function| function.pre_fns.iter()));
    for pre in helpers {
        match pre {
            IrPreFn::MonEval { c_name, context, .. } | IrPreFn::RealEval { c_name, context, .. }
...
fn helper_names(source: &IrWaitSrc) -> Vec<&str> {
    match source {
        IrWaitSrc::Evaluated { eval, condition, .. } | IrWaitSrc::EvaluatedReal { eval, condition, .. } => {
            std::iter::once(eval.as_str()).chain(condition.as_deref()).collect()
        }
...
impl Frame<'_, '_> {
    fn wait_event_address(&mut self, event: &IrEventRef) -> Result<String, String> {
        ...
        Ok(self.scalar("llg_event_t*", format!("({address}) ? ({address}) : &{empty}")))
    }
    ...
    pub(super) fn event_specs(&mut self, specs: &[(IrWaitSrc, IrEdge)]) -> Result<String, String> {
        if specs.is_empty() { return Ok("NULL".to_owned()); }
        ...
        for (source, _) in specs {
            for helper in helper_names(source) {
                if let Some(context) = context_for(model, helper) {
                    if let Some(previous) = contexts.iter().find(|item| item.frame() == context.frame()) {
                        if previous != &context { return Err("inconsistent shared event context".to_owned()); }
                    } else {
        ...
        for context in &contexts {
            prepared.push(self.prepare_captures(context.captures().iter()
        ...
        for (source, edge) in specs {
            let edge = match edge {
                IrEdge::Any => "LLG_EV_ANY", IrEdge::Posedge => "LLG_EV_POSEDGE",
            ...
            match source {
                IrWaitSrc::Sig(name) | IrWaitSrc::Real(name) => {
                    ...
                    } else { fields.push(format!(".sig = {}", binding.address)); }
                }
                IrWaitSrc::Event(event) | IrWaitSrc::FilteredEvent { event, .. } => {
                    fields.push(format!(".event = {}", self.wait_event_address(event)?));
                }
                IrWaitSrc::Evaluated { eval, reads, .. } | IrWaitSrc::EvaluatedReal { eval, reads, .. } => {
                    let field = if matches!(source, IrWaitSrc::EvaluatedReal { .. }) {
                        fields.push(".real = 1".to_owned()); "real_eval"
                    } else { "eval" };
                    fields.push(format!(".{field} = {eval}"));
                    if !reads.is_empty() {
                        let dependencies = reads.iter().map(|item| self.dependency(item))
                            .collect::<Result<Vec<_>, _>>()?;
                        let name = self.name("event_dependencies");
                        self.line(format!("llg_wait_dependency_t {name}[] = {{ {} }};", dependencies.join(", ")));
                        fields.push(format!(".dependencies = {name}, .n_dependencies = {}", reads.len()));
                    }
            ...
            let condition = match source {
                IrWaitSrc::Evaluated { condition, .. } | IrWaitSrc::EvaluatedReal { condition, .. } => condition.as_deref(),
        ...
        for name in retained { self.line(format!("llg_frame_retain({name});")); }
        let array = self.name("events");
        self.line(format!("llg_expr_event_spec_t {array}[] = {{ {} }};", entries.join(", ")));
        for context in &contexts {
