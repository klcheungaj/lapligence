...
impl NativeKind {
    pub(super) fn c_type(self) -> &'static str {
        match self {
            Self::String => "llg_string_t",
            Self::Chandle => "void*",
            Self::Process => "llg_process_handle_t*",
        }
    }
...
impl NativeValue {
    ...
    pub(super) fn take_string(&self) -> String {
        format!("llg_string_take({})", self.address)
    }
}
...
impl Frame<'_, '_> {
    ...
    pub(super) fn native_lookup(
        ...
        name: &str,
        kind: NativeKind,
    ) -> Result<NativeBinding, String> {
        if let Some(binding) = self.native_access_binding(name, kind)? {
            return Ok(binding);
        }
        ...
        for scope in self.native_bindings.iter().rev() {
            if let Some(binding) = scope.get(name) {
                if binding.kind != kind {
                    return Err(format!("native type mismatch for {name}"));
                }
        ...
        let object = self
            ...
            .objects
            .iter()
            .find(|object| object.c_name == name);
        let matches = object.is_some_and(|object| {
            matches!(
                (object.ty, kind),
                (IrObjectType::String, NativeKind::String)
                    | (
        ...
        let string_local = kind == NativeKind::String
            && self.ctx.model.funcs.iter().any(|function| {
                function
                    .locals
                    .iter()
                    .any(|local| local.string && local.c_name() == name)
            });
    ...
    pub(super) fn native_address(
        ...
        if let Some(name) = address.strip_prefix('&') {
            return self.native_lookup(name, kind);
        }
        if let Some(function) = self.ctx.func {
            for (index, formal) in function.formals.iter().enumerate() {
                ...
                if (formal.string || formal.chandle)
                    && expected == kind
                    && ((formal.is_out && address == format!("o{index}"))
                        || (formal.is_ref() && address == format!("r{index}")))
                {
                    return Ok(NativeBinding {
