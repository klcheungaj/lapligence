...
fn numeric_model() -> IrModel {
    ...
    model
        .signals
        .push(IrSignal::new("G_value".to_owned(), None, ty, None).unwrap());
    let formal = IrExpr::new(IrExprKind::FormalRead(0), 65, false, None);
    model.funcs.push(IrFunc::new(
        "f_increment".to_owned(),
        Some(ty),
        vec![IrFormal::new(false, 65, false).unwrap()],
        Vec::new(),
        Vec::new(),
        vec![IrStmt::Return {
            value: Some(Box::new(add(formal, number(1, 65), 65))),
        }],
    ));
...
fn unmigrated_opaque_expression_is_rejected_not_rendered() {
    ...
    model.processes[0].body = vec![IrStmt::Assign {
        lhs: IrLhs::Whole(0),
        nba: false,
        rhs: IrExpr::new(
            IrExprKind::Verbatim {
                code: "old_untracked_result()".to_owned(),
                width: 65,
                signed: false,
            },
            65,
    ...
    let error = super::super::model::render(&execution)
        .unwrap_err()
        .to_string();
...
fn evaluator_callbacks_reject_side_effect_capable_calls() {
    ...
    let error = frame
        .expression(&add(number(1, 65), call, 65))
        .err()
        .unwrap();
    assert!(
        error.contains("side-effect-capable evaluator expressions"),
        "{error}"
    );
}
...
fn event_array_indices_are_owned_and_invalid_handles_are_inert() {
    ...
    model.events.push(IrEvent::new_array(
