...
impl<'a, 'm> Frame<'a, 'm> {
    ...
    fn array_cell(
        ...
        for (expr, (left, right)) in indices.iter().zip(&array.dims) {
            ...
            offsets.push((
                format!("({valid} ? (uint64_t){offset} : 0ULL)"),
        ...
        for (offset, extent) in offsets {
            linear = format!("(({linear}) * {extent}ULL + {offset})");
        }
    ...
    fn selection(
        ...
        Ok(match selection {
            IrElemSel::PackedChain(steps) => {
                let width = steps
                    .last()
                    .ok_or_else(|| "empty packed selection chain".to_owned())?
                    .width;
                let plan = self.scalar(
                    "sv4_select_plan_t",
                    format!("sv4_select_plan_init({storage_width})"),
                );
                for step in steps {
                    let base = self.expression(&step.base)?;
                    self.line(format!(
                        "sv4_select_plan_step(&{plan}, {}, {});",
                        base.code, step.width
                    ));
                    self.discard(base);
                }
                Some(Selection::PackedChain(plan, width))
            }
        ...
        let (binding, valid, selection, net) = match lhs {
            ...
            | IrLhs::IdxPart(index, ..) => {
                ...
                if !signal.net_alias.is_empty() {
                    return Err(pending("true-net-alias writes"));
                }
                let net = signal
                    .net_driver
                    .map(|(group, slot)| (self.ctx.model.net_group(group).c_name.clone(), slot));
                let address = if let Some((name, slot)) = &net {
                    format!("{name}.drivers[{slot}]")
                } else {
            ...
            IrLhs::WholeRef {
                ...
                if binding.width != *width {
                    return Err("local target storage width mismatch".to_owned());
                }
            ...
            IrLhs::Ref {
                ...
                return Ok(Target {
                    ...
                    valid: "1".to_owned(),
                    width: if selection.is_some() { 1 } else { *width },
                    signed: if selection.is_some() { false } else { *signed },
                    selection,
                    net: None,
    ...
    fn select_code(&self, target: &Target, source: &str) -> String {
        match &target.selection {
            ...
            Some(Selection::PackedChain(plan, _)) => {
                format!("sv4_select_plan_read({source}, &{plan})")
            }
            Some(Selection::Bit(index)) => format!("sv4_bit_select({source}, {index})"),
            Some(Selection::Part(left, right)) => {
                format!("sv4_part_select({source}, {left}LL, {right}LL)")
            }
            Some(Selection::Indexed(base, width, neg)) => format!(
                "sv4_idx_part_select_value({source}, {}, {width}, {})",
                base.code,
                u8::from(*neg)
            ),
        }
    ...
    pub(super) fn read_target(&mut self, target: &Target) -> Value {
        if let Some(reference) = &target.reference {
            ...
            // A bit reference records the selected width, while the descriptor
            // still reads the whole caller value. Preserve that storage width
            // and convert two-state views before a selection can supply X for
            // out-of-range positions.
            let whole = if target.binding.two_state {
                let code = format!("sv4_to_two_state({})", whole.code);
                let width = whole.width;
                let signed = whole.signed;
                self.replace(whole, code, width, signed)
            } else {
                whole
            };
        ...
        if matches!(target.selection, Some(Selection::PackedChain(..))) {
            // An invalid unpacked index supplies a default WHOLE element.
            // Inner out-of-range positions still produce X, even when that
            // element's two-state default is zero.
            let whole = self.value(
                format!(
                    "({}) ? sv4_clone({}) : {}",
                    target.valid,
        ...
        let code = if target.width == 0 {
            format!("({}) ? *({}) : 0.0", target.valid, target.binding.address)
        } else {
            format!(
                "({}) ? {} : {}",
                target.valid,
    ...
    pub(super) fn set_selected(&mut self, selection: &Selection, destination: &str, source: &str) {
        self.line(match selection {
            Selection::PackedChain(plan, _) => {
                format!("sv4_select_plan_set(&{destination}, &{plan}, {source});")
            }
            Selection::Bit(index) => {
                format!("sv4_bit_select_set(&{destination}, {index}, {source});")
            }
            Selection::Part(left, right) => {
                format!("sv4_part_select_set(&{destination}, {left}LL, {right}LL, {source});")
            }
            Selection::Indexed(base, width, neg) => format!(
                "sv4_idx_part_select_set_value(&{destination}, {}, {width}, {}, {source});",
                base.code,
                u8::from(*neg)
            ),
        });
    ...
    pub(super) fn store(
        ...
        if let Some(reference) = &target.reference {
            ...
            if nba {
                return Err("sequence locals cannot be nonblocking targets".to_owned());
            }
            self.line(format!(
            ...
            let call = if nba {
                format!(
                    "llg_nba_d_after({}, {}, {ticks});",
                    binding.address, value.code
                )
            } else {
                format!("llg_ba_d({}, {});", binding.address, value.code)
            };
            self.line(call);
            ...
            self.line(match (&target.net, nba) {
                (Some((name, slot)), false) => {
                    format!("llg_net_write(&{name}, {slot}, {});", value.code)
                }
                (Some((name, slot)), true) => format!(
    ...
    pub(super) fn mutation(
        ...
        let current = if mutation.reads_current || mutation.post {
            ...
            self.bindings.last_mut().expect("mutation scope").insert(
                "_llg_mut_current".to_owned(),
                Binding {
        ...
        let result = if mutation.post {
            self.read_binding(
                &self
                    .lookup("_llg_mut_current")
                    .expect("post mutation snapshot"),
            )
        } else if value.width == 0 {
            self.value(value.code.clone(), 0, value.signed)
        } else {
            self.value(
