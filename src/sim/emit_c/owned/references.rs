...
impl Frame<'_, '_> {
    pub(super) fn reference_address(&self, address: &str) -> Result<String, String> {
        ...
        if let Some(function) = self.ctx.func {
            for (index, formal) in function.formals.iter().enumerate() {
                if formal.is_ref() && !formal.string && !formal.chandle && address == format!("r{index}") {
    ...
    pub(super) fn reference_argument(&mut self, lhs: &IrLhs, read: &IrExpr,
        width: u32, signed: bool, two_state: bool,
        ...
        if let IrExprKind::Container(expression) = read.kind() {
          if let IrContainerExpr::Get { container, index } = expression.as_ref() {
            ...
            if matches!(container.kind, IrContainerKind::Queue { .. }) && container.element.is_packed() {
                let index = self.expression(index)?;
                let pointer = self.scalar("llg_ref_t*", format!("llg_ref_queue(&{}, sv4_to_index({}))", container.c_name, index.code));
                self.discard(index);
                return Ok(pointer);
            }
        }
        }
        if let IrLhs::Ref { addr, bit: None, .. } = lhs { return self.reference_address(addr); }
        ...
        let scope = self.scalar("llg_value_scope_t*", "llg_value_scope_begin_object(sizeof(llg_ref_t), NULL)".to_owned());
        let pointer = self.scalar("llg_ref_t*", format!("(llg_ref_t*)llg_value_scope_object({scope})"));
