...
fn member(root: IrLhs, two_state: bool) -> IrLhs {
    IrLhs::PackedSelect {
        target: Box::new(root),
        steps: vec![IrPackedSelect { base: number(0, 32), width: 8 }],
        signed: false,
        two_state,
    }
}
...
fn local_root(name: &str) -> IrLhs {
    IrLhs::WholeRef {
        addr: format!("&{name}"), width: 16, signed: false, two_state: false, shortreal: false,
    }
}

fn read_member() -> IrExpr {
    IrExpr::new(IrExprKind::PartSel {
...
fn callback_member_writes_and_nested_reads_use_private_owners() {
    ...
    model.funcs.push(IrFunc::new(
        "f_member".to_owned(),
        Some(IrType::Packed { width: 8, signed: false, two_state: false }),
        vec![IrFormal::new(false, 16, false).unwrap()], vec![], vec![],
        vec![
            IrStmt::Assign { lhs: member(local_root("a0"), false), rhs: number(7, 8), nba: false },
            IrStmt::Return { value: Some(Box::new(read_member())) },
        ],
    ));
    let expression = IrExpr::new(IrExprKind::CallFn(Box::new(IrCallExpr::new(
        0, vec![IrCallArg::Val(number(0x1234, 16))], IrDepth::PROC, false,
    ))), 8, false, None);
...
fn packed_ref_member_updates_via_the_original_reference_descriptor() {
    ...
    model.funcs.push(IrFunc::new("f_ref".to_owned(), None, vec![formal], vec![], vec![], vec![]));
    let ctx = RCtx { model: &model, func: Some(&model.funcs[0]), sampled: false, activation_label: None };
    let mut frame = Frame::new(&ctx);
    let root = IrLhs::Ref { addr: "r0".to_owned(), width: 16, signed: false,
        two_state: false, const_ref: false, bit: None };
    ...
    assert!(frame.body().contains("llg_ref_write(r0,"));
...
fn whole_signed_member_reads_restore_the_descriptor_sign() {
    ...
    if let IrLhs::PackedSelect { signed, .. } = &mut lhs { *signed = true; }
