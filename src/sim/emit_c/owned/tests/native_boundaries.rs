...
fn native_access_receiver_is_evaluated_once_at_the_use_site() {
    ...
    model.native_accesses[0].receiver = IrChandleExpr::Call {
        function: 0,
        args: vec![],
        depth: IrDepth::PROC,
        receiver: None,
    ...
    let result = frame
        .expression(&IrExpr::new(
            IrExprKind::LocalRead("_llg_access_0".to_owned()),
            129,
            false,
            None,
        ))
    ...
    assert!(
        frame.body().find("f_factory(").unwrap() < frame.body().find("llg_class_field(").unwrap()
    );
}
...
fn semaphore_creation_and_queries_never_request_legacy_fragments() {
    ...
    frame
        ...
        .unwrap();
    let query =
        IrObjectQuery::SemaphoreTryGet(IrChandleExpr::LocalRead("sem".to_owned()), number(2, 32));
    let result = frame
        .expression(&IrExpr::new(
...
fn reference_calls_register_descriptors_and_do_not_copy_back() {
    ...
    let arg = IrCallArg::RefAddr {
        addr: "not_executable_C".to_owned(),
        width: 65,
        signed: false,
        two_state: false,
        const_ref: false,
    ...
    assert!(frame
        .call_values(0, &[arg], IrDepth::PROC)
        .unwrap()
        .is_none());
...
fn const_ref_callback_reads_are_live_and_not_writable_calls() {
    ...
    model.funcs.push(IrFunc::new(
        "f_read".to_owned(),
        Some(IrType::packed(65, false).unwrap()),
        vec![formal],
        vec![],
        vec![],
        vec![IrStmt::Return {
...
fn container_inside_uses_owned_items_and_ordinal_associative_reads() {
    ...
    model.containers.push(IrContainer {
        c_name: "G_set".to_owned(),
        kind: IrContainerKind::Associative {
        ...
        element: IrContainerElement::Packed {
            width: 129,
            signed: false,
            two_state: false,
        },
...
fn native_destination_receiver_retains_its_execution_effects() {
    ...
    model.spawns.push("p".to_owned());
    let execution = ExecutionModel::lower(model).unwrap();
    assert!(execution.processes().iter().any(|process| process
        .effects
        .contains(&crate::sim::execution::ExecutionEffect::RuntimeService)));
}
