...
fn container_model(element: IrContainerElement) -> IrModel {
    ...
    model.containers.push(IrContainer {
        c_name: "G_values".to_owned(), element,
        kind: IrContainerKind::Dynamic, initial_size: None,
    });
    model
}
...
fn native_return_slot_precedes_argument_cleanup_mark() {
    ...
    let result = frame.native_call(0, &[], IrDepth::PROC, NativeKind::String).unwrap();
    let address = result.address.clone();
    frame.native_discard(result);
    let source = frame.body();
    assert!(source.find("llg_value_scope_begin_object").unwrap() < source.find("llg_value_scope_mark()").unwrap());
...
fn string_formals_are_copied_into_registered_callee_owners() {
    ...
    formal.string = true;
    let mut function = IrFunc::new("f_text".to_owned(), None, vec![formal], vec![], vec![], vec![
        IrStmt::Object(IrObjectStmt::StringAssignLocal("_ret".to_owned(), IrStringExpr::FormalRead(0))),
        IrStmt::Return { value: None },
    ]);
    function.ret_string = true;
    model.funcs.push(function);
    let function = &model.funcs[0];
    let ctx = RCtx { model: &model, func: Some(function), sampled: false, activation_label: None };
    ...
    let returned = source.find("llg_string_t _llg_returned = llg_string_clone(").unwrap();
    assert!(returned < source.rfind("llg_value_scopes_end_since(_llg_frame_base)").unwrap());
...
fn native_model_lifecycle_uses_typed_notification_and_release() {
    ...
    model.objects.push(IrObject { c_name: "G_text".to_owned(), ty: IrObjectType::String,
        initial: Some(IrStringExpr::Literal(b"abc".to_vec())) });
    model.objects.push(IrObject { c_name: "G_process".to_owned(), ty: IrObjectType::Process, initial: None });
...
fn container_read_keeps_index_owner_until_after_runtime_call() {
    let model = container_model(IrContainerElement::Packed { width: 129, signed: false, two_state: false });
    let ctx = RCtx { model: &model, func: None, sampled: false, activation_label: None };
    let mut frame = Frame::new(&ctx);
    let expression = IrExpr::new(IrExprKind::Container(Box::new(IrContainerExpr::Get {
        container: 0, index: Box::new(number(2, 32)),
    })), 129, false, None);
...
fn file_scan_uses_registered_source_and_typed_borrowed_target() {
    ...
    model.funcs.clear(); model.processes.clear(); model.spawns.clear();
    let ctx = RCtx { model: &model, func: None, sampled: false, activation_label: None };
    let mut frame = Frame::new(&ctx);
    let input = IrFileInput::ScanString {
        ...
        targets: vec![IrFileInputTarget::Packed {
            lhs: Box::new(IrLhs::Whole(0)), width: 65, signed: false, two_state: false,
        }],
    };
    ...
    assert!(source.find("llg_value_scope_begin_object").unwrap() < source.find("llg_string_scanf(").unwrap());
    assert!(source.find("llg_string_scanf(").unwrap() < source.find("llg_value_scope_end(").unwrap());
}
...
fn enum_navigation_releases_receiver_members_step_and_result() {
    ...
    let ctx = RCtx { model: &model, func: None, sampled: false, activation_label: None };
    let mut frame = Frame::new(&ctx);
    let expression = IrExpr::new(IrExprKind::EnumMethod(Box::new(IrEnumQuery {
        method: IrEnumMethod::Next, receiver: Some(Box::new(number(3, 65))),
        step: Some(Box::new(number(1, 32))),
        members: vec![IrEnumMember { value: number(3, 65), name: b"three".to_vec() },
                      IrEnumMember { value: number(7, 65), name: b"seven".to_vec() }],
        default: number(0, 65),
    })), 65, false, None);
    let result = frame.expression(&expression).unwrap(); frame.discard(result);
...
fn native_read_only_callbacks_reject_random_state_mutation() {
    ...
    let error = frame.system_expression(&IrSysFunc::Urandom { seed: None }, &number(0, 32)).err().unwrap();
...
fn generic_queue_queries_use_the_generic_runtime_layout() {
    ...
    model.containers[0].kind = IrContainerKind::Queue { maximum_elements: None };
    let ctx = RCtx { model: &model, func: None, sampled: false, activation_label: None };
    let mut frame = Frame::new(&ctx);
    let query = IrArrayQuery { kind: IrArrayQueryKind::Size, dimension: None,
        target: IrArrayQueryTarget::Container { container: 0,
            dimensions: vec![IrArrayDimension { left: None, right: None }] } };
    let result = frame.array_query(&query, &number(0, 32)).unwrap(); frame.discard(result);
