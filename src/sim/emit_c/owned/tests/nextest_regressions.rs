...
fn disabled_timed_call_does_not_copy_out_its_staged_value() {
    ...
    let output = IrLhs::WholeRef { addr: "o0".to_owned(), width: 65, signed: false,
        two_state: false, shortreal: false };
    model.funcs.push(IrFunc::new("f_cancelled".to_owned(), None,
        vec![IrFormal::new(true, 65, false).unwrap()], vec![], vec![], vec![
            IrStmt::ActivationScope { target, exit: "task_exit".to_owned(), body: vec![
                IrStmt::Assign { lhs: output.clone(), rhs: number(42, 65), nba: false },
                IrStmt::Delay { ticks: IrDelay::Constant(2) },
                IrStmt::Assign { lhs: output, rhs: number(99, 65), nba: false },
            ] },
        ]));
    model.processes[0].body = vec![
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(7, 65), nba: false },
        IrStmt::ActivationScope { target, exit: "call_exit".to_owned(), body: vec![
            IrStmt::Call(IrCall::new(0, vec![IrCallArg::OutAddr("&result".to_owned())], IrDepth::PROC,
                vec![("result".to_owned(), 0, None)], vec![(IrLhs::Whole(0), "result".to_owned(), 65, false)])),
        ] },
        display_value(),
    ];
    model.processes.push(IrProcess::new("p_disable".to_owned(), "disable".to_owned(),
        IrShape::RunOnce, vec![], vec![
            IrStmt::Delay { ticks: IrDelay::Constant(1) },
...
fn forward_jump_exits_inner_activation_but_preserves_outer_locals() {
    ...
    model.processes[0].body = vec![IrStmt::ActivationScope {
        target: outer, exit: "outer_exit".to_owned(), body: vec![
            IrStmt::DeclLocal { name: "kept".to_owned(), width: 65, signed: false,
                two_state: false, init: Some(Box::new(number(23, 65))) },
            IrStmt::ActivationScope { target: inner, exit: "inner_exit".to_owned(), body: vec![
                IrStmt::DeclLocal { name: "discarded".to_owned(), width: 65537, signed: false,
                    two_state: false, init: Some(Box::new(number(1, 65537))) },
                IrStmt::Goto("escaped".to_owned()),
            ] },
            IrStmt::Label("escaped".to_owned()),
            IrStmt::DisableTarget { target: inner },
            IrStmt::Assign { lhs: IrLhs::Whole(0), nba: false,
...
fn inertial_statement_compiles_and_commits_after_its_delay() {
    let mut model = numeric_model();
    model.processes[0].body = vec![
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(0, 65), nba: false },
        IrStmt::InertialAssign { lhs: IrLhs::Whole(0), rhs: number(42, 65),
            delay: IrTransitionDelay { rise: 2, fall: 2, turn_off: 2 } },
        IrStmt::Delay { ticks: IrDelay::Constant(1) }, display_value(),
        IrStmt::Delay { ticks: IrDelay::Constant(2) }, display_value(),
    ];
...
fn typed_strobe_callback_observes_the_nba_value() {
    ...
    model.processes[0].pre_fns.push(IrPreFn::DisplayEval {
        c_name: "print_current".to_owned(), time_unit_fs: 1,
        args: vec![IrDisplayArg::Packed(IrExpr::new(IrExprKind::SigRead(0), 65, false, None))],
    });
    model.processes[0].body = vec![
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(0, 65), nba: false },
        IrStmt::MonitorSet { strobe: true, fmt: "\"%0d\"".to_owned(), eval: "print_current".to_owned(),
            n_args: 1, reads: vec![], default_radix: IrDisplayRadix::Decimal,
            scope: "regression".to_owned(), descriptor: None },
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(42, 65), nba: true },
        IrStmt::Delay { ticks: IrDelay::Constant(1) },
    ];
...
fn force_evaluator_owns_its_result_and_release_keeps_variable_value() {
    ...
    model.processes[0].pre_fns.push(IrPreFn::ForceEval {
        c_name: "force_value".to_owned(), value: number(42, 65), real: false,
    });
    model.processes[0].body = vec![
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(7, 65), nba: false },
        IrStmt::Force { lhs: IrLhs::Whole(0), value: number(42, 65), eval: "force_value".to_owned(), reads: vec![] },
        IrStmt::Assign { lhs: IrLhs::Whole(0), rhs: number(99, 65), nba: false },
        display_value(),
        IrStmt::Release { lhs: IrLhs::Whole(0) }, display_value(),
    ];
...
fn loop_budget_is_not_charged_for_the_final_false_condition() {
    ...
    frame.statement(&IrStmt::While { cond: number(0, 1), body: vec![] }).unwrap();
