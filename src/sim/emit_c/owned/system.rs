    ...
    pub(super) fn system_expression(&mut self, function: &IrSysFunc, expr: &IrExpr) -> Result<Value, String> {
        let result = match function {
            ...
            IrSysFunc::Urandom { seed } => {
                ...
                } else { self.value("llg_urandom()".to_owned(), 32, false) }
            }
            IrSysFunc::UrandomRange { max, min } => {
                let max = self.expression(max)?;
                let minimum = if let Some(min) = min { self.expression(min)? }
                    else { self.value("sv4_from_u64(0, 32, 0)".to_owned(), 32, false) };
                let code = format!("llg_urandom_range({}, {}, {})", max.code, minimum.code, u8::from(min.is_some()));
                let result = self.replace(max, code, 32, false);
                self.discard(minimum);
                result
            }
            IrSysFunc::Clog2(arg) | IrSysFunc::Rtoi(arg) | IrSysFunc::Itor(arg)
            | IrSysFunc::RealToBits(arg) | IrSysFunc::BitsToReal(arg)
            | IrSysFunc::ShortRealToBits(arg) | IrSysFunc::BitsToShortReal(arg) => {
            ...
            IrSysFunc::BitQuery { kind, arg } => {
                ...
                let code = match kind {
                    ...
                    IrBitQuery::IsUnknown => format!("sv4_from_u64(sv4_is_unknown({}), 1, 0)", value.code),
                };
                self.replace(value, code, expr.width, expr.signed)
            }
            IrSysFunc::Math { kind, args } => {
                let name = match kind {
                    IrMathFunc::Ln => "log", IrMathFunc::Log10 => "log10", IrMathFunc::Exp => "exp",
                    IrMathFunc::Sqrt => "sqrt", IrMathFunc::Pow => "pow", IrMathFunc::Floor => "floor",
                    IrMathFunc::Ceil => "ceil", IrMathFunc::Sin => "sin", IrMathFunc::Cos => "cos",
                    IrMathFunc::Tan => "tan", IrMathFunc::Asin => "asin", IrMathFunc::Acos => "acos",
                    IrMathFunc::Atan => "atan", IrMathFunc::Atan2 => "atan2", IrMathFunc::Hypot => "hypot",
                    IrMathFunc::Sinh => "sinh", IrMathFunc::Cosh => "cosh", IrMathFunc::Tanh => "tanh",
                    IrMathFunc::Asinh => "asinh", IrMathFunc::Acosh => "acosh", IrMathFunc::Atanh => "atanh",
                };
                let mut values = Vec::new();
                for arg in args { values.push(self.expression(arg)?); }
                let arguments = values.iter().map(Value::real).collect::<Vec<_>>().join(", ");
