...
impl Frame<'_, '_> {
    fn qualifier_diagnostic(&mut self, check: &IrUniquePriorityCheck, count: &str, fallback: bool) {
        let location = match check.origin() {
            Some(crate::sim::semantic::Origin::Source {
                path, line, column, ..
            }) => format!("{path}:{line}:{column}"),
            Some(crate::sim::semantic::Origin::Synthetic { reason }) => {
        ...
        if let Some(kind) = check.kind_code() {
            self.line(format!(
                "llg_unique_priority_check({kind}, {count}, {}, {});",
                u8::from(fallback),
    ...
    pub(super) fn qualified_if(
        ...
        for (i, (cond, _)) in branches.iter().enumerate() {
            if check.is_priority() {
                self.line(format!("if ({selected} < 0) {{"));
            }
            let hit = self.condition(cond)?;
            self.line(format!(
                "if ({hit}) {{ ++{count}; if ({selected} < 0) {selected} = {i}; }}"
            ));
            if check.is_priority() {
                self.line("}");
            }
    ...
    pub(super) fn qualified_case(
        ...
        for (index, item) in items
            .iter()
            .enumerate()
            .filter(|(_, item)| !item.exprs.is_empty())
        {
            ...
            self.line(format!(
                "if ({hit}) {{ ++{count}; if ({selected} < 0) {selected} = {index}; }}"
            ));
            if check.is_priority() {
                self.line("}");
            }
