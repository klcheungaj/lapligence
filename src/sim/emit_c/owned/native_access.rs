...
impl Frame<'_, '_> {
    fn access_address(&mut self, name: &str) -> Result<Option<(String, IrClassFieldType)>, String> {
        let Some(access) = self.ctx.model.native_accesses.iter().find(|a| a.name == name).cloned() else {
            return Ok(None);
        };
        ...
        let (address, ty) = match access.kind {
            IrNativeAccessKind::ClassField { class, field } => {
                ...
                (format!("&llg_class_field({receiver}, {class}, {field})->value.{member}"), ty)
            }
            IrNativeAccessKind::InterfaceMember { interface, member } => {
                ...
                (format!("llg_vif_member({receiver}, {interface}, {member}, {})", c_string_literal(site)),
                    IrClassFieldType::Packed { width: metadata.width, signed: metadata.signed, two_state: metadata.two_state })
            }
        };
        let c_type = match ty {
            IrClassFieldType::Packed { .. } => "sv4_t*", IrClassFieldType::Real { .. } => "double*",
            IrClassFieldType::String => "llg_string_t*", IrClassFieldType::Chandle => "void**",
        };
    ...
    pub(super) fn resolve_lookup(&mut self, name: &str) -> Result<Binding, String> {
        if let Some((address, ty)) = self.access_address(name)? {
            let (width, signed, two_state, shortreal) = match ty {
                IrClassFieldType::Packed { width, signed, two_state } => (width, signed, two_state, false),
                IrClassFieldType::Real { shortreal } => (0, false, false, shortreal),
                _ => return Err("native object member used as packed storage".to_owned()),
            };
            return Ok(Binding { address, width, signed, two_state, shortreal, automatic: false });
        }
        self.lookup(name).ok_or_else(|| pending(&format!("unresolved local read {name}")))
    }

    pub(super) fn native_access_binding(&mut self, name: &str, kind: NativeKind) -> Result<Option<NativeBinding>, String> {
        let Some((address, ty)) = self.access_address(name)? else { return Ok(None); };
        if !matches!((ty, kind), (IrClassFieldType::String, NativeKind::String) | (IrClassFieldType::Chandle, NativeKind::Chandle)) {
            return Err("native member storage type mismatch".to_owned());
        }
        Ok(Some(NativeBinding { address, kind, automatic: false }))
    }

    pub(super) fn construct_class(&mut self, index: usize) -> Result<String, String> {
        if self.read_only_callback { return Err(pending("class construction in read-only callbacks")); }
        ...
        for statement in &allocation.body { self.statement(statement)?; }
