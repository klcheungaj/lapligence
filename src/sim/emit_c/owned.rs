...
impl Value {
    fn real(&self) -> String {
        if self.width == 0 { self.code.clone() }
        else { format!("sv4_to_real({})", self.code) }
    }
    fn truth(&self) -> String {
        if self.width == 0 { format!("llg_real_to_bool({})", self.code) }
        else { format!("sv4_to_bool({})", self.code) }
    }
    fn unknown_truth(&self) -> String {
        if self.width == 0 { "0".to_owned() }
        else { format!("(!sv4_to_bool({0}) && sv4_is_unknown({0}))", self.code) }
    }
}
...
impl<'a, 'm> Frame<'a, 'm> {
    ...
    fn reserve(&mut self, width: u32, signed: bool) -> Value {
        ...
        Value { code: format!("_llg_t[{slot}]"), width, signed, fill: None, slot: Some(slot) }
    }
    fn value(&mut self, code: String, width: u32, signed: bool) -> Value {
        if width == 0 {
            return Value { code: self.scalar("double", code), width, signed, fill: None, slot: None };
        }
    ...
    fn convert(&mut self, value: Value, width: u32, signed: bool, two_state: bool, shortreal: bool) -> Value {
        let code = if width == 0 {
            ...
            format!("sv4_from_real({}, {width}, {})", value.code, u8::from(signed))
        } else {
    ...
    fn lookup(&self, name: &str) -> Option<Binding> {
        for bindings in self.bindings.iter().rev() {
            if let Some(binding) = bindings.get(name) { return Some(binding.clone()); }
        }
        for func in &self.ctx.model.funcs {
            if let Some(local) = func.locals.iter().find(|local| local.c_name() == name) {
                return Some(Binding { address: format!("&{name}"), width: if local.real { 0 } else { local.width() },
                    signed: local.signed(), two_state: local.two_state, shortreal: local.shortreal,
                    automatic: false });
            }
        }
        for (index, signal) in self.ctx.model.signals.iter().enumerate() {
            ...
            if signal.c_name == name && (!signal.omit || signal.net_driver.is_some()) {
                return Some(Binding { address: format!("&{name}"), width: signal.ty.width(),
                    ...
                    shortreal: matches!(signal.ty, IrType::Real { shortreal: true }), automatic: false });
            }
        }
        None
    }
        ...
        if let Some(func) = self.ctx.func {
            for (index, formal) in func.formals.iter().enumerate() {
                if address == format!("o{index}") && formal.is_out && !formal.is_ref() {
                    return Ok(Binding { address: address.to_owned(), width: if formal.real { 0 } else { formal.width },
                        signed: formal.signed, two_state: formal.two_state, shortreal: formal.shortreal,
                        automatic: true });
                }
            }
        }
    ...
    fn read_binding(&mut self, binding: &Binding) -> Value {
        let code = if binding.width == 0 { format!("*({})", binding.address) }
            else { format!("sv4_clone({})", binding.address) };
    ...
    fn local(&mut self, name: &str, width: u32, signed: bool, two_state: bool, init: Option<&IrExpr>) -> Result<(), String> {
        ...
        let binding = Binding { address: address.clone(), width, signed, two_state, shortreal: false, automatic: true };
        // Make the default visible to self-referential initializers.
        if width != 0 {
            self.line(format!("sv4_replace({address}, {});", super::expressions::packed_default(width, signed, two_state)));
        }
        self.bindings.last_mut().expect("frame always has a binding scope").insert(name.to_owned(), binding.clone());
        if let Some(init) = init {
            let value = self.expression(init)?;
            let value = self.convert(value, width, signed, two_state, false);
            if width == 0 { self.line(format!("*({address}) = {};", value.code)); }
            else { self.line(format!("sv4_move({address}, &{});", value.code)); }
            self.discard(value);
        }
        Ok(())
    }
    ...
    pub(super) fn body(&self) -> &str { &self.code }
}
