...
impl Codegen<'_> {
    ...
    pub(super) fn emit_mailbox_object_initializers(&mut self) -> Result<(), String> {
    ...
    pub(super) fn is_mailbox_expr(&self, _path: &str, node: NodeId) -> bool {
        self.mailbox_descriptor_name(node).is_some()
    }

    pub(super) fn mailbox_element_for_decl(&self, node: NodeId) -> IrMailboxElement {
    ...
    pub(super) fn lower_mailbox_expr(
        &mut self,
        path: &str,
        node: NodeId,
    ...
    pub(super) fn lower_mailbox_method(
        &mut self,
        path: &str,
        node: NodeId,
