...
impl<'c, 'a> EmitCtx<'c, 'a> {
    ...
    fn lower_cycle_delay(&mut self, h: NodeId, count_node: NodeId) -> Result<Vec<IrStmt>, String> {
        if self.in_final {
            return Err(format!(
...
impl EmitCtx<'_, '_> {
    ...
    fn capture_cycle_lhs(&self, h: NodeId, lhs: IrLhs) -> (Vec<IrStmt>, IrLhs) {
