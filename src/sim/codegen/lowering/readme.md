...
- **`assertions.rs`:** lower the bounded concurrent-assertion sequence
  automaton, legal multiclock `##0`/`##1` clock-flow, default-clock
  inheritance, named instance expansion, sampled property composition,
  conditional/abort controls, and ordered local match-item effects over
  per-attempt sequence state.
