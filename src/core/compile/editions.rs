...
pub(super) fn edition_diagnostics(
    ...
    for node in &snapshot.semantic_nodes {
        ...
        if edition == LanguageEdition::Verilog2001
            ...
            && matches!(
                node.operation,
                SemanticOperation::LogicalImplication
                    | SemanticOperation::LogicalEquivalence
                    | SemanticOperation::WildcardEqual
                    | SemanticOperation::WildcardNotEqual
                    | SemanticOperation::PreIncrement
                    | SemanticOperation::PreDecrement
                    | SemanticOperation::PostIncrement
                    | SemanticOperation::PostDecrement
                    | SemanticOperation::StreamLeft
                    | SemanticOperation::StreamRight
                    | SemanticOperation::Inside
                    | SemanticOperation::AssignmentPattern
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        if !matches!(
            token.kind,
            LexicalKind::String | LexicalKind::Number | LexicalKind::Operator
        ) && token.text.len() > 1
        ...
        if edition == LanguageEdition::Verilog2001
            && token.kind == LexicalKind::Operator
            && matches!(
                token.text.as_str(),
                "++" | "--"
                    | "+="
...
mod tests {
    ...
    fn capability_tables_are_sorted_unique_and_disjoint() {
        for table in [
            VERILOG_SYSTEM_NAMES,
            SYSTEMVERILOG_SYSTEM_NAMES,
            VERILOG_KEYWORDS,
            SYSTEMVERILOG_KEYWORDS,
        ] {
            assert!(table.windows(2).all(|w| w[0] < w[1]));
        }
        assert!(VERILOG_SYSTEM_NAMES
            .iter()
            .all(|s| !listed(SYSTEMVERILOG_SYSTEM_NAMES, s)));
        assert!(VERILOG_KEYWORDS
            .iter()
            .all(|s| !listed(SYSTEMVERILOG_KEYWORDS, s)));
    }
