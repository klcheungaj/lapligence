//! Pure four-state value operations and frontend-neutral constant decoding.
//!
//! Slang resolves semantic constants before the native snapshot is copied.
//! This module provides the exact owned value representation and arithmetic
//! shared by semantic lowering, lint, and simulation.
...
pub enum ElabError {
    /// A constant form this value layer does not support.
    Unsupported(String),
...
/// Decode an owned frontend-neutral value for elaboration and simulation.
pub fn decode_value_data(value: &ValueData, size: i32) -> Result<Val, ElabError> {
    match value {
        ...
        ValueData::Scalar(sc) => {
            let b = match *sc {
                ScalarValue::Zero => Bit::Zero,
                ScalarValue::One => Bit::One,
                ScalarValue::Z => Bit::Z,
                ScalarValue::X | ScalarValue::DontCare => Bit::X,
            };
        ...
        ValueData::Vector {
            ...
            let width = usize::try_from(*bit_width).map_err(|_| {
                ElabError::Unsupported("vector width does not fit usize".to_string())
            })?;
            if width > MAX_RESOLVED_BITS {
                return Err(ElabError::Unsupported(format!(
                    "stored vector value is too wide ({width} bits)"
                )));
            }
            let expected_words = width.div_ceil(64);
            if value_words.len() != expected_words || unknown_words.len() != expected_words {
                return Err(ElabError::Unsupported(
                    "stored vector planes do not match the declared width".to_string(),
                ));
            }
            let bits = (0..width)
                .rev()
                .map(|index| {
                    ...
                    match (
                        unknown_words[word] & mask != 0,
                        value_words[word] & mask != 0,
                    ) {
                ...
                .collect();
            Ok(Val::Bits(Value::from_bits(bits, *is_signed)))
        }
...
fn parse_radix(s: &str, base_bits: usize, size: i32) -> Result<Value, ElabError> {
...
fn stored_width(size: i32, default: usize) -> Result<usize, ElabError> {
...
fn parse_decimal(s: &str, size: i32) -> Result<Value, ElabError> {
    let width = stored_width(size, 64)?;
    let text = s.trim();
