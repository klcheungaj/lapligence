...
static int check_file_io(void) {
    ...
    llg_file_input_target_t targets[] = {
        { LLG_FILE_INPUT_PACKED, &number_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_PACKED, &mixed_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_STRING, NULL, NULL, &word , 0},
    };
    ...
    llg_file_input_target_t source_targets[] = {
        { LLG_FILE_INPUT_PACKED, &source_number_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_PACKED, &source_unknown_ref, NULL, NULL , 0},
    };
    ...
    llg_file_input_target_t numeric_targets[] = {
        { LLG_FILE_INPUT_PACKED, &binary_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_PACKED, &octal_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_PACKED, &auto_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_PACKED, &unsigned_ref, NULL, NULL , 0},
        { LLG_FILE_INPUT_REAL, NULL, &real_number, NULL , 0},
    };
    ...
    llg_file_input_target_t embedded_target_desc = {
        LLG_FILE_INPUT_STRING, NULL, NULL, &embedded_target, 0
    };
    ...
    if (llg_file_seek(binary, test_value(sv4_from_u64(0, 32, 1)), test_value(sv4_from_u64(0, 32, 1))) != 0)
    ...
    if (llg_file_seek(binary, test_value(sv4_from_u64(0, 32, 1)), test_value(sv4_from_u64(0, 32, 1))) != 0 ||
        llg_file_read_array(binary, ascending, 8, 0, 0, 4, ascending_dims, 1,
                            0, test_value(sv4_from_u64(0, 1, 0)), 0, test_value(sv4_from_u64(0, 1, 0))) != 4 ||
    ...
    sv4_destroy(&number);
    sv4_destroy(&mixed);
    sv4_destroy(&source_number);
    sv4_destroy(&source_unknown);
    sv4_destroy(&binary_number);
    sv4_destroy(&octal_number);
    sv4_destroy(&auto_number);
    sv4_destroy(&unsigned_number);
    sv4_destroy(&wide);
    for (int i = 0; i < 4; i++) {
        sv4_destroy(&descending[i]);
        sv4_destroy(&ascending[i]);
    }
    llg_rt_cleanup();
    return 0;
}
