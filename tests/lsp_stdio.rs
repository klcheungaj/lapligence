...
impl LspProcess {
    ...
    fn wait_for_server_request_where<F>(&mut self, predicate: F) -> Result<Value, String>
    where
        F: Fn(&Value) -> bool,
    {
    ...
    fn wait_for_notification_where<F>(
        &mut self,
        method: &str,
        predicate: F,
    ) -> Result<Value, String>
    ...
    fn initialize(&mut self, roots: &[(&str, &Path)], options: Value) -> Result<Value, String> {
    ...
    fn send_watch_event(&mut self, path: &Path, event_type: u8) -> Result<(), String> {
        self.send_notification(
...
fn spawn_with_log_file_and_tmpdir(cwd: &Path, log_file: &Path, tmpdir: &Path) -> LspProcess {
    let mut child = Command::new(env!("CARGO_BIN_EXE_llg_ls"))
        ...
        .stderr(Stdio::inherit())
        .spawn()
        .expect("spawn llg LSP server with custom TMPDIR");
    let stdout = child.stdout.take().expect("capture llg stdout");
    let stdin = child.stdin.take().expect("capture llg stdin");
    LspProcess {
        child,
