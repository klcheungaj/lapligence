...
fn packed_callback_activations() {
    sim_cli::run_case(
        "group1_repairs",
        "packed_callback_activations",
        "packed callbacks passed\n",
        "",
        &[],
    );
}
...
fn packed_formal_recursion() {
    sim_cli::run_case(
        "group1_repairs",
        "packed_formal_recursion",
        "packed recursion passed\n",
        "",
        &[],
    );
}
...
fn packed_reference_activations() {
    sim_cli::run_case(
        "group1_repairs",
        "packed_reference_activations",
        "packed references passed\n",
        "",
        &[],
    );
}
...
fn packed_formal_member_views() {
    sim_cli::run_case(
        "group1_repairs",
        "packed_formal_member_views",
        "packed member views passed\n",
        "",
        &[],
    );
}
...
fn packed_formal_copyout_capture() {
    sim_cli::run_case(
        "group1_repairs",
        "packed_formal_copyout_capture",
        "packed copyout capture passed\n",
        "",
        &[],
    );
}
...
fn packed_const_reference_remains_read_only() {
    for optimized in [false, true] {
        let output = sim_cli::invoke_with_env(
            "group1_repairs",
            "packed_const_reference_rejected",
            optimized,
            &[],
            &[],
            &[],
        ...
        assert!(
            stderr.contains("cannot assign to read-only variable")
