...
// keys; an untagged union pattern selects one member.
module tb;
    typedef struct {
        logic [63:0] upper;
        bit [63:0] lower;
    ...
    unpacked_union128_t union_logic_selected = '{
        logic_value: 128'h0123456789abcdef_fedcba9876543210
    };
    unpacked_union128_t union_bit_selected = '{bit_value: {128{1'bx}}};
    unpacked_union512_t union_wide_selected = '{logic_value: {
    ...
    generate
        if (1) begin : generated_scope
            ...
            unpacked_union128_t generated_union = '{logic_value: {
    ...
    initial begin
        ...
        if (union_logic_selected.logic_value
            ...
            $display("FAIL unpacked_union_patterns");
            $finish;
        end

        $display("PASS unpacked_aggregate_assignment_patterns");
        $finish;
    end
endmodule
