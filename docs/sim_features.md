...
for goto/nonconsecutive repetition, invocation-scoped `first_match` (including
all earliest tied endpoints), physical-time multiclock boundaries, precise
packed read/write prefixes, and antecedent-local transfer into consequent
attempts. Regression sources are in `tests/sim_review_batch4.rs` and the
standalone container probe. They have not been executed for this correction.
No feature checkbox or complete-task acceptance is inferred from this source
change. Unsupported contexts below remain unsupported; in particular this does
not add non-packed queue refs, general aggregate mailbox payloads, multi-cycle
sequence combinators, or the full temporal-property/action-control surface.
...
| 27 | Partial | Net aliases | Canonical packed bit networks, driver/force integration and visible alias cells are present. Alias reads are now side-effect free; net-delay commits publish alias views and notify their waiters. These R27/R28 source corrections have unexecuted regressions in `sim_review_batch2.rs`; full alias/driver acceptance remains pending. Dynamic selects, aggregate members and switch-level/resistive aliasing remain outside the bounded implementation. |
...
| 43 | Partial | Reference subroutine arguments | Typed `ref`/`const ref` descriptors alias whole packed variables and runtime-selected elements of fixed unpacked arrays, including one-time index binding, forwarded aliases, bounded automatic calls and suspension observation. Packed queue-element refs retain shared cells after removal/replacement and release them on return or cancellation. Other aggregate and resizable-array lifetime contexts remain incomplete. Packed bit/part-select actuals are language-illegal for 2009; they must not be listed as supported. |
