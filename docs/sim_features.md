...
- 🟨 **Observe/reactive/preponed regions** — §1800-2009 4.4 **[SV-2005]** typed runtime regions, explicit sensitivity-wait migration, fixed-point re-entry and immutable observation views are present; bounded concurrent assertions sample packed predicates in Preponed, resolve attempts in Observed and dispatch actions in Reactive (`sim_concurrent_assertions.rs`), clocking input sampling uses the preponed/observed regions, and program processes use the Reactive/Re-Inactive/Re-NBA set. Deferred assertions, public VPI constructs, and unsupported property forms remain unsupported
...
| Assertions | bounded concurrent assert/assume/cover instances with one explicit signal clock, simple packed implications, asynchronous single-signal `disable iff`, sampled attempts, vacuity accounting and Reactive actions; deferred forms, general sequences/properties and assertion control tasks remain outside the subset | §1800-2009 ch16, 20.11 | [SV-2005] |
...
| 66 | Partial | Assertions and sampled values | Immediate `assert`/`assume`/`cover` evaluate once with four-state truth, explicit or standard default actions, labels and optimizer-preserved cover callbacks (`sim_partial_features/assertions.rs`); concurrent instances now cover one explicit signal clock, simple packed `|->`/`|=>` properties, Preponed sampling, asynchronous single-signal `disable iff`, overlapping attempts, vacuity accounting and Reactive actions (`sim_concurrent_assertions.rs`). Deferred forms, general sequences/properties, `expect`, assertion-control tasks and sampled-value functions including `$sampled/$rose/$fell/$stable/$changed/$past` and the 2009 global-clocking forms remain. |
